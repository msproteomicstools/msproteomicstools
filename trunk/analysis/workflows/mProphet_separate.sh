...
do
    if [ -s $f ]
    then 
    bsub -n $OMP_NUM_THREADS -R "rusage[mem=${memusage}]" "R --slave --args bin_dir=/cluster/apps/openms/openswath-testing/mapdiv/scripts/mProphet/ \
      ...
      /cluster/apps/openms/openswath-testing/mapdiv/scripts/mProphet/mProphet.R > ${f}_mprophet.mProphet" \
      2>&1 > /dev/null;
    else
...
python /cluster/apps/openms/openswath-testing/mapdiv/scripts/fdr_cutoff.py $outname ${outname}_fdr.csv m_score DECOY_ $fdr_cutoff FALSE TRUE 
...
outname=${file_basename}_mProphet_input_merged.csv
for f in *_.short_format.csv; do
    head -n 1 $f > $outname
    break
done
...
# run mprophet and feed the results back into the featureXML with a specified FDR
bsub -n $OMP_NUM_THREADS -R "rusage[mem=${memusage}]" "R --slave --args bin_dir=/cluster/apps/openms/openswath-testing/mapdiv/scripts/mProphet/ \
  mquest=${outname} workflow=LABEL_FREE num_xval=$mpr_num_xval \
  run_log=FALSE write_classifier=1 write_all_pg=${write_all_pg} help=0 project=${outname}_mprophet < \
  /cluster/apps/openms/openswath-testing/mapdiv/scripts/mProphet/mProphet.R > ${outname}_mprophet.mProphet" \
  2>&1 > /dev/null;
